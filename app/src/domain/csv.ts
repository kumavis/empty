...
export function toCsv(scenario: Scenario, result: ScenarioResult, generatedAt: string): string {
  ...
  const inputs: [string, string | number, string][] = [
    ...
    ['Savings held abroad (USD)', scenario.usSavings,
      'Sending any of it to Japan is a remittance; the reverse direction is not'],
    ['Covered asset value at departure (USD)', scenario.coveredAssetValue,
      'ITA art. 60-2(1) tests the VALUE of securities held, against the 100m yen threshold'],
